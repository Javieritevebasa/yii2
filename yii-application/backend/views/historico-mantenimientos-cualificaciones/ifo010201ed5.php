...
$this->title = 'Listado de personal cualificado Inspectores';
...
<div class="historico-mantenimientos-cualificaciones-index container-fluid">
    ...
    <table class="table table-condensed table-bordered table-fixedHeader" >
    	<thead>
	    	<tr>
	    		<th colspan="3"></th>
	    		<th colspan="<?=count($cualificaciones)?>" class="text-center">LISTADO DE INSPECTORES CUALIFICADOS<span class="pull-right">Impreso IFO-01-02-01 Ed.5</span></th>
	    	</tr>
	    	<tr>
	    		<th colspan="3"></th>
	    	...
	    	<tr>
	    		...
	    		<th colspan="<?=count($cualificaciones)-6?>" class="text-center">OTRAS ACTUACIONES</th>
	    	</tr>
	
	    	<tr>
	    		<th width='0.5em'>Estación<input type="text" class="form-control input-sm inputSearch"></th>
    			<th width='5%'>Apellidos, nombre<input type="text" class="form-control input-sm inputSearch"></th>
    			...
    			<?php foreach ($cualificaciones as $key => $cualificacion) : ?>
					<th style="vertical-align: middle; text-align: center !important;"><?= $cualificacion->nombre; ?></th>
				<?php endforeach ?>
	    	</tr>
    	</thead>
    	<tbody>
	<?php foreach ($usuarios as $key => $usuario) : ?>
		<tr>
			...
			<?php
				...
				if (count($usuario->getEstarEspecializado()->where(['cualificadoComo' => 2])->andWhere(['in','idEspecialidad', [10,18,20]])->all()) > 0){
				//if (count($usuario->estarEspecializado)>0){
					$userCualificaciones= ArrayHelper::merge($userCualificaciones, ArrayHelper::index($usuario->getEstarEspecializado()->where(['cualificadoComo' => 2])->andWhere(['in','idEspecialidad', [10,18,20]])->all(), 'idEspecialidad'));
				}
			?>
