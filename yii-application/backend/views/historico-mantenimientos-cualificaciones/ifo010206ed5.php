...
$this->title = 'Plan Supervisiones in situ Inspectores';
...
<div class="historico-mantenimientos-cualificaciones-index">
    ...
    <table class="table  table-bordered table-condensed table-fixedHeader" >
		...
		<tr>
			<th colspan="3"></th>
    		<th colspan="<?=count($cualificaciones)?>" class="text-center">PLAN DE SUPERVISIONES IN SITU A INSPECTORES<span class="pull-right">Impreso IFO-01-02-06 Ed.5</span></th>
    	</tr>
    	<tr>
    		<th colspan="3"></th>
    	...
    	<tr>
    		...
    		<th colspan="1" class="text-center">INSPECCIONES TÉCNICAS NO PERIÓDICAS</th>
    		<th colspan="1" class="text-center">OTRAS ACTUACIONES</th>
    	</tr>    		
    	<tr>
    		...
    		<?php foreach ($cualificaciones as $key => $cualificacion) : ?>
				<th width='<?= 85/count($cualificaciones)?>%' style='vertical-align: middle; text-align: center !important'><?= $cualificacion->nombre; ?></th>
	...
	<?php foreach ($usuarios as $key => $usuario) : ?>
		<tr>
			...
			<?php
				$userCualificaciones = [];
				//print_r($usuario->getEstarCualificado()->where(['cualificadoComo' => 2])->all());die();
				if (count($usuario->getEstarCualificado()->where(['cualificadoComo' => 2])->all()) > 0){
					$userCualificaciones = ArrayHelper::index($usuario->getEstarCualificado()->where(['cualificadoComo' => 2])->all(), 'idGrupoCategorias');
				}
				if (count($usuario->getEstarEspecializado()->where(['cualificadoComo' => 2])->all()) > 0){
				//if (count($usuario->estarEspecializado)>0){
					$userCualificaciones= ArrayHelper::merge($userCualificaciones, ArrayHelper::index($usuario->getEstarEspecializado()->where(['cualificadoComo' => 2])->all(), 'idEspecialidad'));
				}
			?>
	...
	<tfoot>
		<tr>
			<td colspan="17" style="text-align:left !important;">
				<p>M2/M3: Incluido transporte escolar y de menores en inspecciones periódicas. La escolar de M1 la hará un inspector cualificado para escolar en M2/M3.</p>
