...
$this->title = 'Listado de personal cualificado Responsables Técnicos';
...
<div class="historico-mantenimientos-cualificaciones-index container-fluid">
    ...
    <table class="table table-condensed table-bordered table-fixedHeader" >
    	<thead>
	    	<tr>
	    		<th colspan="2"></th>
	    		<th colspan="<?=count($cualificaciones)?>" class="text-center">LISTADO DE RESPONSABLES TÉCNICOS CUALIFICADOS<span class="pull-right">Impreso IFO-01-02-09 Ed.2</span></th>
	    	</tr>
	    	<tr>
	    		<th colspan="2"></th>
	    	...
	    	<tr>
	    		<th colspan="2"></th>
	    		<th colspan="<?=count($cualificaciones)?>" class="text-center">INSPECCIONES TÉCNICAS NO PERIÓDICAS</th>
	    		
	    	</tr>
	
	    	<tr>
	    		<th width='0.5em'>Estación<input type="text" class="form-control input-sm inputSearch"></th>
    			<th width='2%'>Apellidos, nombre<input type="text" class="form-control input-sm inputSearch"></th>
    			<?php foreach ($cualificaciones as $key => $cualificacion) : ?>
					<th style='vertical-align: middle; text-align: center !important'><?= ($cualificacion->id == 6 ? 'Reformas' : $cualificacion->nombre); ?></th>
				<?php endforeach ?>
	    	</tr>
    	</thead>
    	<tbody>
		...
		<tr>
			...
			<?php
				...
				if (count($usuario->estarEspecializado) > 0){
					...
					$userCualificaciones = ArrayHelper::index($usuario->estarEspecializado, 'idGrupoCategorias');
